    ...
    if len(sys.argv)>2:
       ...
       # ps=poisson.rvs(rate, size=T)
        ...
        for t in range(T):
            ...
            if t>1:
                    subprocess.run('cp -f'+demo_path +'/CI/'+str(t-1)+'/CA-ON.csv ~/caspian/monitoring/data/CA-ON.csv ',shell=True)
                    subprocess.run('cp -f'+demo_path +'/CI/'+str(t-1)+'/US-TEX-ERCO.csv ~/caspian/monitoring/data/US-TEX-ERCO.csv ',shell=True)
